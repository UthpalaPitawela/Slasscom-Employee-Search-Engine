...
import { searchByMemberName } from '../../services/queries';
...
describe('searchByMemberName', () => {
  it('fetches successfully data from an API', async () => {
    const mockData = {
      data: {
        ...
        }/* Mock your expected response here */,
      },
    };

    (API.graphql).mockImplementation(() => Promise.resolve(mockData));

    const result = await searchByMemberName('fullName', 'Priyantha Miller');
  ...
  it('handles API errors', async () => {
    ...
    await expect(searchByMemberName('name', 'John')).rejects.toThrow(mockError);
  });
});
